...
function getLocalDateMidnight(utcDate: Date, offsetMinutes: number): Date {
  const dateStr = getLocalDateString(utcDate, offsetMinutes)
  return new Date(`${dateStr}T00:00:00Z`)
}

// Find local Monday for a given date
function getLocalMonday(utcDate: Date, offsetMinutes: number): Date {
...
): { currentStreak: number; hasMissedYesterday: boolean } {
  const now = new Date()
...
router.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const freezes = await prisma.freezeHistory.findMany({
      where: { userId },
    ...
    return sendRouteError(res, error, 'Failed to retrieve streak data.')
  }
})
...
router.post('/session', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    return sendRouteError(res, error, 'Failed to record study session.')
  }
})

// POST /api/streaks/use-freeze - Consume a freeze to protect missed yesterday
router.post('/use-freeze', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.uid
  if (!userId) return res.status(401).json({ error: 'Unauthorized' })
  ...
  try {
    ...
    const existingFreeze = await prisma.freezeHistory.findFirst({
      where: {
        userId,
        frozenDate: {
    ...
    return sendRouteError(res, error, 'Failed to apply streak freeze.')
  }
})

// POST /api/streaks/let-go - Reset streak to 0 and let it go
router.post('/let-go', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.uid
  if (!userId) return res.status(401).json({ error: 'Unauthorized' })
  ...
  try {
    ...
    return sendRouteError(res, error, 'Failed to reset streak.')
  }
})
