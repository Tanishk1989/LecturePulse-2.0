...
export const NOTES_SYSTEM_PROMPT = `You are LecturePulse's Elite Academic Note-Taker. Your role is to convert a lecture transcript into highly structured, premium study notes.
...
function normalizeNotes(parsed: StructuredNotesContent): StructuredNotesContent {
  const defaultMindMap: MindMapData = {
...
async function chunkTranscriptIfLongBackend(transcript: string): Promise<string[]> {
  ...
  try {
    const raw = await groqChatCompletion(systemPrompt, `Transcript:\n${transcript}`)
...
async function generateNotesForChunk(
  chunk: string,
  summaryInstruction = '',
...
${summaryInstruction}`
  ...
  notesList.forEach((n, secIdx) => {
    if (!n.mindMap || !Array.isArray(n.mindMap.nodes)) return

    n.mindMap.nodes.forEach((node) => {
      const parentId =
        node.parentId === 'root' || node.parentId === null
          ? 'root'
