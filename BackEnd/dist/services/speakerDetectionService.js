...
exports.detectSpeakersInSegments = detectSpeakersInSegments;
const groq_1 = require("./groq");
const BATCH_SIZE = 40;
function parseSpeakerRole(value) {
    if (value === 'professor' || value === 'student' || value === 'unknown') {
        return value;
    }
    return 'unknown';
}
function classifySegmentHeuristic(text) {
    ...
    if (trimmed.length >= 200)
        return 'professor';
    if (trimmed.length <= 50 && lower.includes('?'))
        return 'student';
    return 'unknown';
}
function applyHeuristicLabels(segments) {
    return segments.map((segment) => ({
        ...segment,
        speaker: segment.speaker ?? classifySegmentHeuristic(segment.text),
    }));
}
function mergeVoiceClusters(segments) {
    const clusters = segments.map((s) => s.voiceCluster).filter((c) => c === 0 || c === 1);
    if (clusters.length < 4)
        return segments;
    const durationByCluster = { 0: 0, 1: 0 };
    ...
    return segments.map((segment) => {
        if (segment.voiceCluster !== 0 && segment.voiceCluster !== 1)
            return segment;
        const voiceSpeaker = segment.voiceCluster === professorCluster ? 'professor' : 'student';
        return {
            ...segment,
...
async function labelBatchWithLlm(segments, subject) {
...
async function detectSpeakersInSegments(segments, options) {
    if (segments.length === 0)
        return [];
